      ...
      <View>
        {
          quizz?.questoes?.map((que, index) => {
            return <View key={que.id} className="mb-4">
              <Text className="font-bold text-lg mb-1">{(index + 1) + " - " + que.pergunta}</Text>
              {que.alternativas.map(alt => {
                return <TouchableOpacity key={alt.id} className="bg-amber-300/50 p-1 mb-2 rounded-md flex-row items-center">
                  <Ionicons name="checkmark-circle" color={"rgb(245 158 11)"} size={24} />
